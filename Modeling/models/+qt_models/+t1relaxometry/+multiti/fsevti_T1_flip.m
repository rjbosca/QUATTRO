classdef fsevti_T1_flip < sevti & mrimagingprops
    ...
    properties (Constant)
        ...
        modelName = 'Multiple TI SE (with inversion FA)';
        ...
        %
        %       'FA'            Inversion pulse flip angle (units: deg.)
        nlinParams = {'S0','T1','FA'};
    ...
    methods
        ...
        function obj = fsevti_T1_flip(varargin)
        ...
        end %fsevti_T1_flip.fsevti_T1_flip
    ...
    methods
        ...
        function val = get.modelFcn(obj)
            val = @(x,xdata) fse_vti(x(:),xdata);
            if ~obj.usePolarityCorrection
                val = @(x,xdata) abs(val(x,xdata));
            end
        end %fsevti_T1_flip.get.modelFcn
    ...
    methods (Hidden)
        ...
        %   employed when "autoGuess" is enabled (i.e. true).
            ...
            [obj.tiInvThreshCache.internal,x0] = restore_ir( abs(obj.y), nls );

            % Update the parameter guess if "autoGuess" is enabled
            val.S0 = x0(1);
            val.T1 = x0(2);

        end %fsevti_T1_flip.processGuess
