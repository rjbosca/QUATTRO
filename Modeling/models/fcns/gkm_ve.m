function ydata = gkm_ve(x,xdata,vif,hct)
%gkm_ve  Calculates tissue uptake cureve from the general kinetic model
%
%   Ct = gkm_ve(x,t,Cp,hct) returns the tissue uptake curve using the general
...
%# FILENAME  : gkm_ve.m
    ...
    % increments
    H = diff(xdata(1:2)) * conv(vif, x(1)*exp(-x(1)/(x(2)+eps) * xdata));
