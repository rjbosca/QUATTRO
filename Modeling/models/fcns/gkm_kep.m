function ydata = gkm_kep(x,xdata,vif,hct)
%gkm_kep  Calculates tissue uptake cureve from the general kinetic model
%
%   Ct = gkm_kep(x,t,Cp,hct) returns the tissue uptake curve using the general
...
%                                   (see note below).
%
%       x(2)           kep          Efflux [Gd] transfer rate between the EES
%                                   and capillary space in the same units as
%                                   Ktrans
%
...
%   checking is performed and the time vector and, hence, the VIF and tissue
%   data are assumed to be sampled uniformly.
%
...
%# FILENAME  : GKM.m
    ...
    % increments
    H     = diff(xdata(1:2)) * conv(vif, x(1)*exp(-x(2)*xdata));
