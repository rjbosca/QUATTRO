function [ H, machine ] = ReadAnalyzeHeader(fileprefix)

% Read Analyze format data header (*.hdr)
%
% [ H, machine ] = ReadAnalyzeHeader(fileprefix)
%
...
%
% H.hdr - a struct, all fields returned from the header.
...
fid = fopen(sprintf('%s.hdr',fileprefix),'r');

if fid < 0,
    error('Unable to open file %s.hdr\n\n',fileprefix);
else
    fclose(fid);
    fid = fopen(sprintf('%s.hdr',fileprefix),'r',machine);
    H.hdr = read_header(fid);
    H.fileprefix = fileprefix;
    fclose(fid);
    
    if ~isequal(H.hdr.hk.sizeof_hdr,348),
    ...
    if ~isequal(H.hdr.hk.sizeof_hdr,348),
        % Now echo warning
        error('Error reading %s Analyze format.\nSize of header not equal to 348 bytes!\n\n',machine);
    end
