classdef unit
    ...
    % Syntax:
            ...
            %       and sqrt
            % The following triginometric functions will accept variables
            % with units:
...
%            if A.name==B.name
    ...
    % Plotting methods
        ...
        elseif rows==2 && isa(varargin{1},'unit') && isa(varargin{2},'struct')
            ...
            error('Cannot plot the "y" variable perhaps because it is a structure and field. Change to a simple variable name.')
        end % if
        % If an output argument was requested, construct it
        if nargout>0
            handles=h;
