function qim_slider_series_value_postset(src,eventdata)
%qim_slider_series_value_postset  PostSet event for QUATTRO's series slider "Value" property
%
%   qim_slider_series_value_postset(SRC,EVENT) handles changes to the "Value"
%   property of QUATTRO's series slider UI using the slider handle (SRC) and
%   event data (EVENT) objects.
    ...
    % variables
    hSlider = findobj(obj.hFig,'Tag','edit_series');

    % Set the new value
    set(hSlider,'String',num2str(sliderVal));

end %qim_slider_series_value_postset